...
class _Whitening(nn.Module):
    ...
    def __init__(self, num_features, momentum=0.1, affine=False, track_running_stats=True, eps=1e-3):
        super(_Whitening, self).__init__()

        if affine:
            raise Exception("Affine transformation is not implemented in Whitening layers")
        ...
        self.group_size = self.num_features # Do not use grouping
    ...
    def forward(self, x):
        ...
        f_cov_shrinked = (1 - self.eps) * f_cov + self.eps * torch.eye(self.group_size,
                                                                       out=torch.cuda.FloatTensor() if torch.cuda.is_available() else torch.FloatTensor()).repeat(
            self.num_groups, 1, 1)

        if not self.training and self.track_running_stats:  # for inference
            f_cov_shrinked = (1 - self.eps) * self.running_variance + self.eps * torch.eye(self.group_size,
                                                                                           out=torch.cuda.FloatTensor() if torch.cuda.is_available() else torch.FloatTensor()).repeat(
                self.num_groups, 1, 1)

        inv_sqrt = torch.inverse(torch.cholesky(f_cov_shrinked)).contiguous().view(self.num_features, self.group_size,
                                                                                   1, 1)

        decorrelated = conv2d(xn, inv_sqrt, groups=self.num_groups)

        if self.training and self.track_running_stats:
            self.running_mean = torch.add(self.momentum * m.detach(), (1 - self.momentum) * self.running_mean,
                                          out=self.running_mean)
            self.running_variance = torch.add(self.momentum * f_cov.detach(),
                                              (1 - self.momentum) * self.running_variance, out=self.running_variance)

        return decorrelated


class Whitening2d(_Whitening):
    def _check_input_dim(self, input):
        if input.dim() != 4:
